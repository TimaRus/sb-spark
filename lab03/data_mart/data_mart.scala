...
object data_mart extends App {
  ...
  val esOpts:HashMap[String,String] = HashMap(
    ...
    , "es.net.http.auth.user" -> props.getProperty("timofey.melnikov")
    , "es.net.http.auth.pass" -> props.getProperty("D5QMspzo")
  )
  ...
  val pgOpts:HashMap[String,String] = HashMap(
    ...
    , "user" -> props.getProperty("timofey_melnikov")
    , "password" -> props.getProperty("D5QMspzo")
  )
  ...
  @transient lazy val pgTargetDb:Connection = DriverManager.getConnection(pgTargetUrl, pgOpts("timofey_melnikov"), pgOpts("D5QMspzo"))
